...
export default function AdminDashboard() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
        <div className="text-center">
          <Loader size="lg" />
          <p className="mt-4 text-sm font-medium text-slate-400">Loading pending tailors...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6">
      <div className="mx-auto max-w-4xl">
        <h1 className="mb-6 text-3xl font-bold text-slate-50">Admin Dashboard</h1>
        <p className="mb-6 text-slate-400">Review and approve pending tailor profiles</p>
          ...
          <div className="space-y-4">
            {tailors.map((tailor) => (
              <Card key={tailor._id} className="p-6">
                <div className="space-y-4">
                  <div>
                    <h3 className="text-xl font-bold text-slate-50">{tailor.shopName}</h3>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">{tailor.user?.email || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-300">Location</p>
                      <p className="text-sm text-slate-400">{tailor.location || "N/A"}</p>
                    </div>
                    {tailor.experience && (
                      <div>
                        <p className="text-sm font-medium text-slate-300">Experience</p>
                        <p className="text-sm text-slate-400">{tailor.experience} years</p>
                      </div>
                    )}
                    {tailor.priceRange && (
                      <div>
                        <p className="text-sm font-medium text-slate-300">Price Range</p>
                        <p className="text-sm text-slate-400">{tailor.priceRange}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {tailor.services && tailor.services.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-slate-300 mb-2">Services</p>
                  ...
                  <div className="pt-3 border-t border-slate-700">
                    <button
                      ...
                      className="w-full rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-lg transition-all duration-200 hover:bg-green-500 hover:shadow-xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
